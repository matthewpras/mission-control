...
- [x] **Code Quality:** Refactor `script.js` to migrate agent state management from `localStorage` to Supabase/SQLite to prevent state desync.
- [ ] **Test Coverage:** Implement a baseline test suite for `scripts/health_monitor.py` and agent lifecycle events.
- [x] **Prompt Quality:** Scrub all Heartbeat and Workflow prompts for legacy Ollama references; modernize templates for current models.
...
- [x] **Data Integrity:** Verify Supabase connection; initialize DB via `setup_supabase.sql` and migrate local state to the cloud.
